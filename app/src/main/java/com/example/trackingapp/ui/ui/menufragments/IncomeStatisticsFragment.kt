...
class IncomeStatisticsFragment : Fragment() {
    ...
    private fun loadPieChartData() {
        ...
        transactionViewModel.getTransactionExpense.observe(viewLifecycleOwner) {incomeTransaction ->
            ...
            for (category in TransactionCategory.TRANSACTION_CATEGORIES) {
                ...
                val categoryList =
                    incomeTransaction.filter { it.category.categoryDescription == category.categoryDescription }
                val totalCategory = categoryList.sumOf { it.amount }
                val percentageValue = (totalCategory / incomeAmount).toFloat()
                if (totalCategory > 0) {
    ...
    private fun setupPieChart() {
        ...
        binding.expenseChart.centerText = "Income By Category"
